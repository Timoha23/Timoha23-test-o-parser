...
1. Для того, чтобы бот смог отправлять вам уведомления необходимо отправить боту сообщение "/start", чтобы он мог отправлять вам уведомления, далее внести свой id в файл .env в переменную USER_IDS. Можно внести несколько id, но нужно это сделать через запятую.
